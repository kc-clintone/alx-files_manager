...
const generateThumbnail = async (filePath, size) => {
  ...
  const fileId = job.data.fileId || null;                                       const userId = job.data.userId || null;
...
userQueue.process(async (job, done) => {
  ...
  try {
    const mailSubject = 'ALX File Manager by Kc-clintone';
    const mailContent = [
      '<div>',
      '<h3>Hello {{user.name}},</h3>',
      'Welcome to <a href="https://github.com/kc-clintone/alx-files_manager">',
      'ALX-Files_Manager</a>, ',
      'A very simple file management API ',
      'Hqppy hacking.',
      '</div>',
    ].join('');
