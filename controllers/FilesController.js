...
import { tmpdir } from 'os';                          import { promisify } from 'util';
...
const ROOT_FOLDER_ID = 0;                             const DEFAULT_ROOT_FOLDER = 'files_manager';
const mkdirAsync = promisify(mkdir);                  const writeFileAsync = promisify(writeFile);
const statAsync = promisify(stat);
const actualPathAsync = promisify(realpath);
...
export default class FilesController {
  static async postUpload(req, res) {
    ...
    const b64data = req.body && req.body.data ? req.body.data : '';
    ...
    if (type !== VALID_FILE_TYPES.folder) {
      ...
      await writeFileAsync(localPath, Buffer.from(b64data, 'base64'));
  ...
  static async getFile(req, res) {
    ...
    if (file.type === VALID_FILE_TYPES.folder) {
      res.status(400).json({ error: 'This folder is empty' });
      return;
    }
    let myFilePath = file.localPath;
    if (size) {
      myFilePath = `${file.localPath}_${size}`;
    }
    ...
    const absolutePath = await actualPathAsync(filePath);
